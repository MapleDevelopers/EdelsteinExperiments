...
{
    public class RawPacketIncoming : IPacket, IPacketReader
    {
        ...
        public RawPacketIncoming(byte[] buffer)
        {
            _stream = new MemoryStream(buffer);
            _reader = new BinaryReader(_stream);
        }

        public RawPacketIncoming() : this(Array.Empty<byte>()) { }
        ...
        public string ReadString(short? length = null)
            => StringEncoding.GetString(_reader.ReadBytes(length ?? ReadShort()));
        public byte[] ReadBytes(short length)
            => _reader.ReadBytes(length);

        public Vector2 ReadVector2()
            => new Vector2(ReadShort(), ReadShort());
        public DateTime ReadDateTime()
            => DateTime.FromFileTimeUtc(ReadLong());
    }
}
